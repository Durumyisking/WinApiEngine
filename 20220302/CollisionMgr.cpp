...
void CCollisionMgr::CollisionGroupupdate(GROUP_TYPE _eLeft, GROUP_TYPE _eRight)
{
	...
	{
		// 충돌체 보유 X
		if (vecLeft[i]->GetColliderVector().empty())
		{
			continue;
		}
		...
		for (size_t j = 0; j < vecRight.size(); ++j)
		{
			// 충돌체 보유X || 자기 자신과의 충돌
			if (vecLeft[i]->GetColliderVector().empty() ||
				vecRight[j]->GetColliderVector().empty() ||
				vecLeft[i] == vecRight[j])
			{
				continue;
			}
			for (size_t k = 0; k < vecLeft[i]->GetColliderVector().size(); k++)
			{
				for (size_t l = 0; l < vecLeft[j]->GetColliderVector().size(); l++)
				{
					CCollider* pLeftCol = vecLeft[i]->GetColliderVector()[k];
					CCollider* pRightCol = vecRight[j]->GetColliderVector()[l];
					...
					if (IsCollision(pLeftCol, pRightCol))
					{
						...
						if (iter->second)
						{
							...
							if (vecLeft[i]->IsDead() || vecRight[j]->IsDead())
							{
								...
								vecLeft[i]->GetColliderVector()[k]->OnCollisionExit(pRightCol);
								vecRight[j]->GetColliderVector()[l]->OnCollisionExit(pLeftCol);
								iter->second = false;
							}
							else
							{
								// vecleft와 vecright가 충돌중이라는 정보 전달
								vecLeft[i]->GetColliderVector()[k]->OnCollision(pRightCol);
								vecRight[j]->GetColliderVector()[l]->OnCollision(pLeftCol);
							}
						}
						else
						{
							...
							if (!vecLeft[i]->IsDead() && !vecRight[j]->IsDead())
							{
								...
								vecLeft[i]->GetColliderVector()[k]->OnCollisionEnter(pRightCol);
								vecRight[j]->GetColliderVector()[l]->OnCollisionEnter(pLeftCol);
								iter->second = true;
